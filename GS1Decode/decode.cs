...
namespace GS1Decode
{
    ...
    public class GS1DecodeEngine
    {
        ...
        private Dictionary<string, Response> data = new Dictionary<string, Response>();

        /// <summary>
        /// The raw view of the data in the string
        /// </summary>
        public Dictionary<string, Response> Data
        {
            get
            {
                return data;
        ...
        private Dictionary<string, AII> response;
        ...
        {
            AI ai;
            if (AI.AIMaster.TryGetValue(id, out ai))
            {
                aiinfo.Add(id, new AII { ai = id, minLength = minLength, maxLength = maxLength, data_title = ai.data_title, description = ai.description, fnc1 = ai.fnc1, regex = ai.regex });
            }
        }
        ...
        private static void AddAI(string id, int length)
        {
            ...
            if (AI.AIMaster.TryGetValue(id, out ai))
            {
                aiinfo.Add(id, new AII { ai = id, minLength = length, maxLength = length, data_title = ai.data_title, description = ai.description, fnc1 = ai.fnc1, regex = ai.regex });
            }
        }
        ...
        public void ParseData(string s, char fnc1)
        {
            ...
            {
                ai.Append(s[index++]);
                if (!aiinfo.TryGetValue(ai.ToString(), out info))
                {
                    continue;
                }
                if (info != null)
                {
                    ...
                    try
                    {
                        ...
                        String aistr = ai.ToString();
                        Response response = new Response();
                        response.ai = info.ai;
                        response.value = value.ToString();
                        response.description = info.description;
                        response.data_title = info.data_title;
                        response.error = "";
                        var match = Regex.Match(aistr + response.value, info.regex, RegexOptions.IgnoreCase);
                        if (!match.Success)
                        {
                            response.error = "Invalid AI Format. [" + aistr + "]" + response.value + " - " + response.description;
                        }
                        data.Add("AI-" + aistr, response);
                        float nvalue = getNumericValue(aistr);
                        if (aistr.Length == 4)
                        {
                            data["AI-" + aistr].value = nvalue.ToString();
                        }
        ...
        {
            if (data.TryGetValue(aiCode, out Response t))
            {
                return t.value;
            }
            else
        ...
        public float getNumericValue(string aiCode)
        {
            ...
            if (data.TryGetValue(aiCode, out Response t))
            {
                if (aiCode.Length == 4)
                {
                    ...
                    try
                    {
                        double conversion = Math.Pow(10, Convert.ToDouble(new string(aiCode[3], 1)));
                        float workingValue = Convert.ToSingle(t.value);
                ...
                else
                {
                    switch (aiCode)
                    {
                        case "30":
                        case "37":
                            try { ret = Convert.ToSingle(t.value); }
        ...
        {
            //string t;
            if (!data.TryGetValue("12", out Response t))
                throw new Exception("Application Identifier not found.");

            if (data["12"].value.Substring(4, 2) == "00")//GS1 Date information with 00 
            //as the date is to be interpreted as the last day of the month accounting for 
            {
                string temp = data["12"].value.Substring(0, 4) + "01";
                return DateTime.ParseExact(temp, "yyMMdd", CultureInfo.InvariantCulture).AddMonths(1).AddDays(-1);

            }

            return DateTime.ParseExact(data["12"].description, "yyMMdd", CultureInfo.InvariantCulture);
        }
    }
}
